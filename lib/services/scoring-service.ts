...
import { results, game, player } from "@/models/schema";
import { eq, and, inArray } from "drizzle-orm";
import { calculatePlayerStats as calculateStatsUtil, sortByTieBreakers } from "@/utils/tie-breakers";
import type { PlayerStats, RankedPlayer, LeaderboardEntry } from "@/types/tournament";

export function calculatePlayerScores(results: { player_id: string; placement: number }[]) {
...
export async function calculatePlayerStatsForPhase(
    ...
    const phaseResults = playerResults.filter(r => r.game?.phase_id === phaseId);
...
export async function getLeaderboard(
    ...
    const playerStatsArray = Array.from(playerResultsMap.entries()).map(([playerId, playerResults]) => {
    ...
    const leaderboard: LeaderboardEntry[] = sorted.map((item, index) => {
        const playerData = playerStatsArray.find(p => p.stats.player_id === item.stats.player_id);
        if (!playerData) {
