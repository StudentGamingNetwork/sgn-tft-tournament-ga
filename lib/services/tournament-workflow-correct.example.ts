 ...
 * - Phase 2 : 96 joueurs (96 derniers de P1 - les top 32 P1 passent directement en P3 Master) (1 bracket common)
 * - Phase 3 : 128 joueurs (2 brackets, RESET points)
 *   - Master: 64 joueurs (Top 32 P1 qualifiés directement + Top 32 P2)
...
export async function exampleFullTournamentWorkflow() {
    ...
    const newTournament = await createStandardTournament('Tournoi TFT 2026', '2026');
    ...
    // ============================================
    // ÉTAPE 2 : Importer les 128 joueurs depuis CSV
    // ============================================
...
`.trim();
    ...
    // ============================================
    // PHASE 1 : 128 joueurs, 16 lobbies (8 joueurs par lobby)
    // ============================================
    console.log('\n📍 PHASE 1 - Démarrage...');
    const phase1Result = await startPhase(phase1.id, {
        autoSeed: true,
        lobbyCount: 16, // 128 joueurs / 8 = 16 lobbies
    ...
    // ============================================
    // PHASE 1 → PHASE 2 : Top 32 qualifiés directement pour P3, 96 derniers jouent P2
    // ============================================
    console.log('\n📍 PHASE 2 - Transition depuis Phase 1...');
    const phase2Result = await startPhase2FromPhase1(
        phase1.id,
        phase2.id,
        12 // 96 joueurs / 8 = 12 lobbies
    );
    console.log(`Phase 2: ${phase2Result.games.length} lobbies créés`);
    console.log(`  🎯 ${phase2Result.eliminatedPlayers.length} joueurs qualifiés directement pour P3 Master (top 32 P1)`);
    console.log(`  ✅ ${phase2Result.qualifiedPlayers.length} joueurs participent à P2 (rangs 33-128 de P1)`)
...
export const TOURNAMENT_STRUCTURE = {
    ...
    phase2: {
        ...
        brackets: ['common'],
        lobbies: 12,
        description: '96 derniers de Phase 1 (top 32 qualifiés directement pour P3 Master)',
    },
    phase3: {
        players: 128,
