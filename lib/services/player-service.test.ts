...
describe('playerService', () => {
    ...
    describe('createPlayer', () => {
        ...
        it('utilise 0 comme LP par défaut si non fourni', async () => {
            const mockPlayer = {
                id: 'player-uuid-1',
                name: 'Test Player',
                riot_id: 'TestPlayer#NA1',
            ...
            expect(insertCall.league_points).toBe(0);
        });
    });
    ...
    describe('importPlayersFromCSV', () => {
        ...
        it('met à jour un joueur existant au lieu de le créer', async () => {
            ...
            // Mock pour l'update
            const returningMock = vi.fn().mockResolvedValue([{ ...existingPlayer, ...csvData[0] }]);
            const whereMock = vi.fn().mockReturnValue({ returning: returningMock });
            const setMock = vi.fn().mockReturnValue({ where: whereMock });
            (db.update as any).mockReturnValue({ set: setMock });
        ...
        it('crée une nouvelle équipe si elle n\'existe pas', async () => {
            ...
            const teamReturningMock = vi.fn().mockResolvedValue([{ id: 'team-id', name: 'New Team' }]);
            ...
            const teamValuesMock = vi.fn().mockReturnValue({ returning: teamReturningMock });
            const playerValuesMock = vi.fn().mockReturnValue({ returning: playerReturningMock });
